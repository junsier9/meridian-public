...
def build_manual_signals() -> list[Signal]:
    return [
        Signal(
            signal_id="s1",
...
def main() -> None:
    with runtime_worker_harness(slug="dry-run-driver"):
        engine = RuntimeOrchestrator()
        result = engine.run(
            object_id="ro-aix-demo",
            object_type=ObjectType.ASSET,
            scope="spot+perp",
            signals=build_manual_signals(),
        )
        ...
        payload = {
            ...
            "cadence": {
                ...
                "normal_review_after": str(result.cadence.normal_review_after),
                "deep_review_after": str(result.cadence.deep_review_after),
            },
