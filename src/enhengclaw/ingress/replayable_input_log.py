...
class ReplayableInputLog:
    def __init__(self, root: str | Path | None = None) -> None:
        self.root = (
            Path(root)
            if root is not None
            else Path(__file__).resolve().parents[3] / "artifacts" / "agent_ingress" / "replayable_input_log"
        )
    ...
    def write(
        ...
        *,
        ...
        file_name = f"{stamp}_{self._safe_fragment(input_id)}_{digest[:12]}.json"
