...
from enhengclaw.ingress.shadow_schema import (
    AlchemyBitcoinRpcSchemaValidator,
...
class AlchemyBitcoinShadowConfig:
    api_key_env_var: str = "ALCHEMY_API_KEY"
    provider_id: str = "alchemy.btc.rpc"
    symbol: str = "BTC"
    subject_key: str = "BTC.alchemy.onchain"
...
class AlchemyBitcoinShadowProvider:
    provider_id = "alchemy.btc.rpc"

    def __init__(
        self,
        config: AlchemyBitcoinShadowConfig | None = None,
        *,
        ...
        self.config = config or AlchemyBitcoinShadowConfig()
        ...
        if self.subject_key.as_stable_string() != self.config.subject_key:
            raise ValueError(
                "Alchemy Bitcoin subject_key does not match the canonical stable string for the configured symbol: "
                f"expected '{self.subject_key.as_stable_string()}', observed '{self.config.subject_key}'"
            )
        self.provider_id = self.config.provider_id
        self.validator = AlchemyBitcoinRpcSchemaValidator(
        ...
        self.checkpoint_path = self.state_root / "alchemy_btc_checkpoint.json"
        self._last_height = self._load_checkpoint()
    ...
    async def run(self, stop_event: asyncio.Event) -> None:
        require_active_worker_lease(
            operation="provider.alchemy_bitcoin_shadow.run",
    ...
    async def poll_once(self) -> None:
        require_active_worker_lease(
            operation="provider.alchemy_bitcoin_shadow.poll_once",
            required_capabilities={CAP_PROVIDER_STREAM},
            requested_scope=DEFAULT_SHADOW_INGEST_SCOPE,
            allowed_entrypoints={INGESTION_WORKER_ENTRYPOINT},
        )
        height_request = self._build_request("getblockcount", [])
        height_response = await self._call_with_retry(height_request)
        self._write_validated_response(
            method="getblockcount",
            request_id=height_request["id"],
            response=height_response,
        )

        latest_height = int(height_response["result"])
        detail_written = False
        if self.config.include_block_details:
            detail_written = await self._recover_and_write_block_range(latest_height)
        else:
            self._remember_height(latest_height)

        self.logger.info(
            "Alchemy Bitcoin poll succeeded against %s height=%s detail_written=%s",
    ...
    def _write_validated_response(
        ...
        *,
        ...
        require_active_worker_lease(
            operation="provider.alchemy_bitcoin_shadow.write_validated_response",
            required_capabilities={CAP_PROVIDER_STREAM},
            requested_scope=DEFAULT_SHADOW_INGEST_SCOPE,
            allowed_entrypoints={INGESTION_WORKER_ENTRYPOINT},
        )
    ...
    async def _call_with_retry(self, request_payload: dict[str, object]) -> Any:
        require_active_worker_lease(
            operation="provider.alchemy_bitcoin_shadow.transport_retry",
        ...
        while True:
            try:
                ...
                if max_attempts is not None and attempt > max_attempts:
                    raise RuntimeError(
                        f"Alchemy Bitcoin RPC retries exhausted for {request_payload['method']}"
                    ) from exc
                delay_seconds = self.config.retry_backoff.delay_for_attempt(attempt)
                self.logger.warning(
                    "Alchemy Bitcoin RPC transient failure for %s against %s; retry %s/%s in %.1fs: %s",
    ...
    def _rpc_call_once(self, request_payload: dict[str, object]) -> Any:
        require_active_worker_lease(
            operation="provider.alchemy_bitcoin_shadow.transport_rpc",
            required_capabilities={CAP_PROVIDER_TRANSPORT},
            requested_scope=DEFAULT_SHADOW_INGEST_SCOPE,
            allowed_entrypoints={INGESTION_WORKER_ENTRYPOINT},
        )
    ...
    def _note_success(self) -> None:
        if self._degraded:
            self.logger.info(
                "Alchemy Bitcoin provider recovered after %s consecutive failures against %s",
    ...
    def _note_failure(self, exc: Exception) -> None:
        ...
        self.logger.error(
            "Alchemy Bitcoin poll failed against %s (%s consecutive failures): %s",
            self.redacted_endpoint,
            self._consecutive_failures,
            exc,
        )
    ...
    async def _recover_and_write_block_range(self, latest_height: int) -> bool:
        previous_height = latest_height - 1 if self._last_height is None else self._last_height
        if previous_height > latest_height:
            previous_height = latest_height - 1
        detail_written = False
        for height in range(previous_height + 1, latest_height + 1):
            block_hash = await self._fetch_block_hash(height)
            detail_request = self._build_request("getblock", [block_hash, 1])
            ...
            detail_written = True
            self._remember_height(height)
        if not detail_written:
            self._remember_height(latest_height)
        return detail_written

    async def _fetch_block_hash(self, height: int) -> str:
        hash_request = self._build_request("getblockhash", [height])
        hash_response = await self._call_with_retry(hash_request)
        if not isinstance(hash_response, dict):
            raise FatalTransportError("Bitcoin getblockhash returned a non-object payload")
        block_hash = hash_response.get("result")
        if not isinstance(block_hash, str) or not block_hash.strip():
            raise FatalTransportError("Bitcoin getblockhash returned an empty block hash")
        return block_hash.strip()

    def _remember_height(self, height: int) -> None:
    ...
    def _load_checkpoint(self) -> int | None:
        ...
        try:
            ...
            return None
        height = payload.get("last_height")
