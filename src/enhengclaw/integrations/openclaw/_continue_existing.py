...
class OpenClawContinueExistingRequest:
    ...
    def to_payload(self, *, text_field_name: str) -> dict[str, Any]:
        return {
            "contract_version": self.contract_version,
            "subject": self.subject,
            "scope": self.scope,
            "object_id": self.object_id,
            text_field_name: self.text_value,
...
@dataclass(frozen=True, slots=True)
class OpenClawContinueExistingResponse:
    contract_version: str
    status: str
...
@dataclass(frozen=True, slots=True)
class OpenClawContinueExistingLaneSpec:
    agent_id: str
    contract_version: str
    text_field_name: str
    entrypoint_module: str
    description: str
    missing_object_context_reason: str
    build_observation: Callable[[OpenClawContinueExistingRequest], Any]
    build_compiler_backend: Callable[[OpenClawContinueExistingRequest], Any]
    pipeline_cls: type[Any]
    build_object_context: Callable[..., Any]
    object_context_error_types: tuple[type[BaseException], ...]
    object_type: ObjectType = ObjectType.ASSET


def build_parser(*, description: str) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument("--request", type=Path, required=True, help="Path to the OpenClaw JSON request.")
    parser.add_argument("--response", type=Path, required=True, help="Path to write the OpenClaw JSON response.")
    return parser


def parse_openclaw_continue_existing_request(
    ...
    *,
    ...
    payload_contract_version = str(payload.get("contract_version", "")).strip()
    if payload_contract_version != contract_version:
        raise ValueError(f"OpenClaw request contract_version must be '{contract_version}'")
    ...
    text_value = _require_non_empty_string(payload.get(text_field_name), text_field_name)
    execution_permit_path = _require_non_empty_string(payload.get("execution_permit_path"), "execution_permit_path")
    if "skip_seed" in payload:
        raise ValueError("skip_seed is not supported on the OpenClaw resume-only boundary")
    ...
    return OpenClawContinueExistingRequest(
        ...
        recorded_transcript_path=None if recorded_transcript_path in {None, ""} else str(recorded_transcript_path),
    )
...
def main_for_lane(
    argv: list[str] | None,
    *,
    spec: OpenClawContinueExistingLaneSpec,
    request_loader: Callable[[dict[str, Any]], OpenClawContinueExistingRequest],
    runner: Callable[[OpenClawContinueExistingRequest], OpenClawContinueExistingResponse],
) -> int:
    args = build_parser(description=spec.description).parse_args(argv)
    request_payload: dict[str, Any] | None = None
    exit_code = 0
    try:
        request_payload = json.loads(_read_text_file(args.request))
        request = request_loader(request_payload)
        response = runner(request)
        exit_code = 0 if response.status != "failed" else 1
    except Exception as exc:
        response = OpenClawContinueExistingResponse(
        ...
        exit_code = 1
    _write_text_file(args.response, json.dumps(response.to_payload(), indent=2, sort_keys=True))
    if exit_code != 0:
        print(response.error or f"OpenClaw {spec.agent_id} adapter failed", file=os.sys.stderr)
    return exit_code
...
def _write_text_file(path: Path, content: str, *, encoding: str = "utf-8") -> None:
    ...
    os.makedirs(_runtime_io_path(parent), exist_ok=True)
    with open(_runtime_io_path(path), "w", encoding=encoding) as handle:
...
def run_openclaw_continue_existing(
    ...
    *,
    ...
    observation = spec.build_observation(request)
    execution = spec.pipeline_cls(
        artifact_store=owner_store,
        compiler_backend=spec.build_compiler_backend(request),
    )
    with _override_runtime_environment(runtime_paths):
        ...
        if not runtime.store.exists(request.object_id):
            result = GovernedAgentOrchestrator(runtime=runtime).run_governed_write(
                requested_delegate_id=spec.agent_id,
                ...
                user_intent=_openclaw_user_intent(spec.agent_id),
                constraints=("single_payload", "single_tool_call", "verify_before_finalize"),
                admission_blocked_reason=spec.missing_object_context_reason,
            ...
            return _response_from_result(
                ...
                result=result,
                compiler_artifact_paths=(),
                blocked_reason=spec.missing_object_context_reason,
                quarantine_reason=None,
                runtime_session_path=None,
            )
        try:
            object_context = spec.build_object_context(
                runtime=runtime,
                object_id=request.object_id,
                subject=request.subject,
                scope=request.scope,
            )
        except spec.object_context_error_types as exc:
            blocked_reason = str(exc) or spec.missing_object_context_reason
            result = GovernedAgentOrchestrator(runtime=runtime).run_governed_write(
                requested_delegate_id=spec.agent_id,
                object_id=request.object_id,
                subject=request.subject,
                ...
                user_intent=_openclaw_user_intent(spec.agent_id),
                constraints=("single_payload", "single_tool_call", "verify_before_finalize"),
                admission_blocked_reason=blocked_reason,
                spec_input_payload=observation.to_spec_input_payload(),
                admission_artifacts=(),
            )
        ...
        step_id = _continue_existing_step_id(
...
def resolve_openclaw_artifacts_root(artifacts_root: str | None) -> Path:
    if artifacts_root is not None and artifacts_root.strip():
        ...
        resolved = Path(tempfile.mkdtemp(prefix="ecoc_ce_", dir=_resolve_openclaw_temp_root())).resolve()
...
def _response_from_result(
    *,
    ...
    status = _response_status(result.run_state, execution_status)
    error = result.blocked_reason if status == "failed" else None
    return OpenClawContinueExistingResponse(
        contract_version=spec.contract_version,
        status=status,
        ...
        compiler_artifact_paths=compiler_artifact_paths,
        accepted_signal_ids=tuple(result.accepted_signal_ids),
        blocked_reason=None if status == "failed" else blocked_reason,
        quarantine_reason=None if status == "failed" else quarantine_reason,
        error=error,
        artifacts_root=str(artifacts_root),
    )


def _response_status(run_state: str, execution_status: str | None) -> str:
...
def _continue_existing_step_id(
    *,
    agent_id: str,
    request: OpenClawContinueExistingRequest,
    observation,
    ...
    return f"{agent_id}:{execution_status}:{idempotency_key[:12]}"


def _openclaw_user_intent(agent_id: str) -> str:
    return f"Execute one OpenClaw {agent_id} deployment request against one existing governed object."
...
def _require_non_empty_string(value: Any, field_name: str) -> str:
    ...
    if not text:
        raise MissingExecutionPermitError(f"OpenClaw request requires non-empty {field_name}")
    return text
