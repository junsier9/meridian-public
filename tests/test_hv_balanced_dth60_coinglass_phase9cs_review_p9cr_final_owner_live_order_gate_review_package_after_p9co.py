...
class Phase9CSReviewP9CRFinalOwnerPackageTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temp_dir = Path(tempfile.mkdtemp(prefix="dth60-cg-p9cs-"))
        self.addCleanup(shutil.rmtree, self.temp_dir, ignore_errors=True)

    def test_ready_reviews_p9cr_package_only_without_live_order_authority(self) -> None:
        paths = self._write_ready_p9cr_inputs()

        summary, exit_code = build_phase9cs(
            self._args(paths, output_root=self.temp_dir / "p9cs"),
            now_fn=lambda: datetime(2026, 6, 8, 10, 0, 0, tzinfo=UTC),
        )

        self.assertEqual(exit_code, 0)
        self.assertEqual(summary["contract_version"], P9CS_CONTRACT)
        ...
        self.assertFalse(summary["p9cr_package_sufficient_for_live_order_submission"])
        self.assertFalse(summary["p9cr_package_sufficient_for_candidate_execution"])
        ...
        self.assertFalse(summary["final_gate_actionable_items_satisfied"])
        self.assertEqual(summary["remaining_evidence_gap_count"], 12)
        self.assertEqual(summary["remaining_approval_gap_count"], 7)
        self.assertTrue(summary["eligible_for_future_p9ct_scope_definition"])
        ...
        self.assertEqual(summary["allowed_next_gate"], P9CT_GATE)

        review = _load_json(Path(summary["output_files"]["p9cr_sufficiency_review"]))
        gap_matrix = _load_json(Path(summary["output_files"]["final_gate_gap_matrix"]))
        ...
        self.assertTrue(review["p9cr_package_sufficient_for_p9cs_review"])
        self.assertFalse(review["p9cr_package_sufficient_for_live_order_submission"])
        self.assertFalse(review["final_gate_actionable_items_satisfied"])
        self.assertEqual(gap_matrix["remaining_evidence_gap_count"], 12)
        self.assertEqual(gap_matrix["remaining_approval_gap_count"], 7)
        self.assertTrue(
            non_auth["authorizations"][
                "review_p9cr_final_owner_live_order_gate_review_package"
            ]
        )
    ...
    def test_blocks_when_p9cr_allowed_next_gate_is_not_p9cs(self) -> None:
        paths = self._write_ready_p9cr_inputs()
        p9cr = _load_json(paths["p9cr_summary"])
        p9cr["allowed_next_gate"] = "P9CT_skip_p9cs_review"
        _write_json(paths["p9cr_summary"], p9cr)

        summary, exit_code = build_phase9cs(
            self._args(paths, output_root=self.temp_dir / "wrong-next-gate"),
            now_fn=lambda: datetime(2026, 6, 8, 10, 5, 0, tzinfo=UTC),
        )

        self.assertEqual(exit_code, 2)
        self.assertEqual(summary["status"], "blocked")
        self.assertIn("p9cr_summary_ready_for_p9cs_review", summary["blockers"])
        self.assertFalse(summary["p9cr_package_sufficient_for_p9cs_review"])
    ...
    def test_wrong_owner_decision_blocks_without_candidate_authority(self) -> None:
        ...
        summary, exit_code = build_phase9cs(
            ...
            now_fn=lambda: datetime(2026, 6, 8, 10, 10, 0, tzinfo=UTC),
        )

        self.assertEqual(exit_code, 2)
        self.assertIn("owner_decision_p9cs_review_only_recorded", summary["blockers"])
        self.assertFalse(summary["live_order_gate_approved"])
        self.assertFalse(summary["candidate_execution_authorized"])
        self.assertFalse(summary["target_plan_replacement_authorized"])
        self.assertEqual(summary["fill_count"], 0)

    def test_blocks_if_p9cr_package_contains_live_order_authorization(self) -> None:
        paths = self._write_ready_p9cr_inputs()
        p9cr = _load_json(paths["p9cr_summary"])
        package_path = Path(p9cr["output_files"]["final_owner_live_order_gate_review_package"])
        ...
        summary, exit_code = build_phase9cs(
            ...
            now_fn=lambda: datetime(2026, 6, 8, 10, 15, 0, tzinfo=UTC),
        )

        self.assertEqual(exit_code, 2)
        self.assertIn("p9cr_review_package_ready", summary["blockers"])
        self.assertFalse(summary["live_order_submission_authorized"])
        self.assertFalse(summary["candidate_enter_executor_target_plan_path_authorized"])
        self.assertEqual(summary["orders_submitted"], 0)

    def test_blocks_if_p9cr_non_authorization_was_mutated(self) -> None:
        paths = self._write_ready_p9cr_inputs()
        p9cr = _load_json(paths["p9cr_summary"])
        non_auth_path = Path(p9cr["output_files"]["non_authorization"])
        ...
        summary, exit_code = build_phase9cs(
            ...
            now_fn=lambda: datetime(2026, 6, 8, 10, 20, 0, tzinfo=UTC),
        )

        self.assertEqual(exit_code, 2)
        self.assertIn("p9cr_non_authorization_ready", summary["blockers"])
        self.assertFalse(summary["live_order_submission_authorized"])
        self.assertFalse(summary["candidate_execution_authorized"])
        self.assertEqual(summary["trade_count"], 0)
    ...
    def _args(
        ...
        *,
        output_root: Path,
        owner_decision: str = APPROVE_P9CS_DECISION,
    ) -> Namespace:
        return Namespace(
            output_root=str(output_root),
            project_profile=str(paths["project_profile"]),
            phase9cr_summary=str(paths["p9cr_summary"]),
            owner="rulebook_owner",
            owner_decision=owner_decision,
            owner_decision_source="unit_test",
        )

    def _write_ready_p9cr_inputs(self) -> dict[str, Path]:
        ...
        p9cr_summary, exit_code = build_phase9cr(
            Namespace(
        ...
        self.assertEqual(exit_code, 0)
        paths["p9cr_summary"] = Path(p9cr_summary["output_files"]["summary"])
        return paths
