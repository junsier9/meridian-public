...
class ResearchLeadPromotedAcceptanceTests(unittest.TestCase):
    ...
    def test_definition_declares_promoted_writable_dual_surface_contract(self) -> None:
        prompt_path = Path(RESEARCH_LEAD_AGENT["prompt_path"])
        self.assertTrue(prompt_path.exists())
        self.assertEqual(RESEARCH_LEAD_AGENT["status"], "governed_agent_slice")
        self.assertTrue(RESEARCH_LEAD_AGENT["enabled_under_current_governance"])
        self.assertTrue(RESEARCH_LEAD_AGENT["registry_admission_eligible"])
        self.assertEqual(RESEARCH_LEAD_AGENT["contract_version"], CONTROLLED_AGENT_SLICE_CONTRACT_VERSION)
        self.assertEqual(
            RESEARCH_LEAD_AGENT["promotion_contract_version"],
            CONTROLLED_AGENT_SLICE_PROMOTION_CONTRACT_VERSION,
        )
        self.assertEqual(RESEARCH_LEAD_AGENT["slice_mode"], "continue_existing_object")
        self.assertEqual(
            RESEARCH_LEAD_AGENT["canonical_runtime_boundary"],
            "runtime.continue_existing_from_agent_payloads",
        )
        self.assertEqual(
            RESEARCH_LEAD_AGENT["schema"],
            "enhengclaw.agents.schemas.research_lead.ResearchLeadSignalDraft",
        )
        self.assertEqual(
            RESEARCH_LEAD_AGENT["tool"],
            "enhengclaw.agents.tools.runtime_signal_intake.submit_research_lead_signal",
        )
        review_surface = dict(RESEARCH_LEAD_AGENT["operator_review_surface"])
        self.assertEqual(
            review_surface["schema"],
            "enhengclaw.agents.schemas.research_lead.ResearchLeadDirective",
        )
        self.assertEqual(
            review_surface["tool"],
            "enhengclaw.agents.tools.runtime_session_views.inspect_research_lead_directive",
        )
    ...
    def test_direct_submit_is_rejected_without_owner_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with runtime_worker_harness(slug="research-lead-promoted-slice", scope="spot+perp"):
                runtime.run_new(
                    object_id="research-lead-aix",
                    object_type=ObjectType.ASSET,
                    scope="spot+perp",
                    signals=sample_signals("research-lead-aix"),
                )
                seeded_session = runtime.store.load("research-lead-aix")
                ...
                with self.assertRaises(UnsupportedGovernedDelegateDirectCallError):
                    submit_research_lead_signal(
                        ...
                        signal=ResearchLeadSignalDraft(
                            input_id="research-lead-agent-1",
                            subject="AIX",
                            predicate="next_stage_targeted_refresh",
                            value="next legal stage remains a targeted refresh rather than direct publish",
                            confidence_hint=65,
                        ),
                    )

            updated_session = runtime.store.load("research-lead-aix")
            self.assertEqual(len(updated_session.claims), seeded_claim_count)

    def test_checked_in_governance_exposes_research_lead_as_current_shipped_slice(self) -> None:
        ...
        self.assertIn("research_lead", governance["current_controlled_slice_ids"])
