...
class ValidationAgentPromotedAcceptanceTests(unittest.TestCase):
    ...
    def test_definition_declares_promoted_governed_contract(self) -> None:
        prompt_path = Path(VALIDATION_AGENT["prompt_path"])
        self.assertTrue(prompt_path.exists())
        self.assertEqual(VALIDATION_AGENT["status"], "governed_agent_slice")
        self.assertTrue(VALIDATION_AGENT["enabled_under_current_governance"])
        self.assertTrue(VALIDATION_AGENT["registry_admission_eligible"])
        self.assertEqual(VALIDATION_AGENT["contract_version"], CONTROLLED_AGENT_SLICE_CONTRACT_VERSION)
        self.assertEqual(
            VALIDATION_AGENT["promotion_contract_version"],
            CONTROLLED_AGENT_SLICE_PROMOTION_CONTRACT_VERSION,
        )
        self.assertEqual(VALIDATION_AGENT["slice_mode"], "continue_existing_object")
        self.assertEqual(
            VALIDATION_AGENT["canonical_runtime_boundary"],
            "runtime.continue_existing_from_agent_payloads",
        )
        self.assertEqual(
            VALIDATION_AGENT["schema"],
            "enhengclaw.agents.schemas.validation_agent.ValidationBlockerSignalDraft",
        )
        self.assertEqual(
            VALIDATION_AGENT["tool"],
            "enhengclaw.agents.tools.runtime_signal_intake.submit_validation_signal",
        )
    ...
    def test_direct_submit_is_rejected_without_owner_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with runtime_worker_harness(slug="validation-promoted-slice", scope="spot+perp"):
                runtime.run_new(
                    object_id="validation-agent-aix",
                    object_type=ObjectType.ASSET,
                    scope="spot+perp",
                    signals=sample_signals("validation-agent-aix"),
                )
                seeded_session = runtime.store.load("validation-agent-aix")
                ...
                with self.assertRaises(UnsupportedGovernedDelegateDirectCallError):
                    submit_validation_signal(
                        ...
                        signal=ValidationBlockerSignalDraft(
                            input_id="validation-agent-1",
                            subject="AIX",
                            predicate="publish_gate_hold",
                            value="unresolved validation blockers require the object to remain on hold before publish",
                            confidence_hint=71,
                        ),
                    )

            updated_session = runtime.store.load("validation-agent-aix")
            self.assertEqual(len(updated_session.claims), seeded_claim_count)

    def test_checked_in_governance_exposes_validation_agent_as_current_shipped_slice(self) -> None:
        ...
        self.assertIn("validation_agent", governance["current_controlled_slice_ids"])
    ...
    def test_public_demo_review_gate_block_returns_blocked_after_runtime_write(self) -> None:
        ...
        override = {
            "validation_review": {
                ...
                "working_primary_thesis_id": None,
                "working_opposing_thesis_id": None,
                "decision": "publish",
