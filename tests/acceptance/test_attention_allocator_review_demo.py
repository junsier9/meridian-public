...
class AttentionAllocatorReviewDemoAcceptanceTests(unittest.TestCase):
    ...
    def test_definition_exposes_pending_writable_and_readonly_review_surface(self) -> None:
        self.assertEqual(ATTENTION_ALLOCATOR_AGENT["status"], "promotion_ready_governed_slice")
        self.assertFalse(ATTENTION_ALLOCATOR_AGENT["enabled_under_current_governance"])
        self.assertTrue(ATTENTION_ALLOCATOR_AGENT["writes_to_runtime"])
        review_surface = dict(ATTENTION_ALLOCATOR_AGENT.get("operator_review_surface", {}))
        ...
        self.assertTrue(Path(ATTENTION_ALLOCATOR_AGENT["prompt_path"]).exists())
