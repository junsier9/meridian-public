...
from enhengclaw.agents.tools.runtime_session_views import inspect_risk_governance_review
...
class RiskGovernanceAgentReviewDemoAcceptanceTests(unittest.TestCase):
    ...
    def test_definition_exposes_pending_writable_and_readonly_review_surface(self) -> None:
        self.assertEqual(RISK_GOVERNANCE_AGENT["status"], "promotion_ready_governed_slice")
        self.assertFalse(RISK_GOVERNANCE_AGENT["enabled_under_current_governance"])
        self.assertTrue(RISK_GOVERNANCE_AGENT["writes_to_runtime"])
        review_surface = dict(RISK_GOVERNANCE_AGENT.get("operator_review_surface", {}))
        ...
        self.assertTrue(Path(RISK_GOVERNANCE_AGENT["prompt_path"]).exists())
    ...
    def test_public_demo_matches_direct_tool_output_without_mutating_session(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(payload["review_surface"], "risk_governance_review")
            ...
            direct_review = inspect_risk_governance_review(runtime=runtime, object_id="risk-governance-review-aix")
