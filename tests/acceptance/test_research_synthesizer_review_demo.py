...
class ResearchSynthesizerReviewDemoAcceptanceTests(unittest.TestCase):
    ...
    def test_definition_exposes_pending_writable_and_readonly_review_surface(self) -> None:
        self.assertEqual(RESEARCH_SYNTHESIZER_AGENT["status"], "promotion_ready_governed_slice")
        self.assertFalse(RESEARCH_SYNTHESIZER_AGENT["enabled_under_current_governance"])
        self.assertTrue(RESEARCH_SYNTHESIZER_AGENT["writes_to_runtime"])
        review_surface = dict(RESEARCH_SYNTHESIZER_AGENT.get("operator_review_surface", {}))
        ...
        self.assertTrue(Path(RESEARCH_SYNTHESIZER_AGENT["prompt_path"]).exists())
