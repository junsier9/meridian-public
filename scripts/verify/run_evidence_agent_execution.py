...
SUCCESS_INPUT = ROOT / "fixtures" / "agent_golden" / "evidence_agent" / "success" / "input.json"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Verify the model-backed evidence_agent execution slice.")
    parser.add_argument("--live-smoke", action="store_true", help="Run one explicit live backend smoke if env vars are present.")
    args = parser.parse_args(argv)

    commands = [
        (
            "evidence_agent execution unit tests",
            [
                sys.executable,
                "-m",
                "unittest",
                "tests.test_evidence_agent_execution",
                "tests.test_owner_state",
            ],
        ),
        (
            "evidence_agent execution acceptance tests",
            [
                sys.executable,
                "-m",
                "unittest",
    ...
    for label, command in commands:
        print(f"[evidence-agent-execution] START {label}")
        completed = subprocess.run(command, check=False, cwd=ROOT, env=env)
        if completed.returncode != 0:
            print(f"[evidence-agent-execution] FAIL {label} (exit={completed.returncode})")
            return completed.returncode
        print(f"[evidence-agent-execution] PASS {label}")
    ...
    print("[evidence-agent-execution] ALL GATES PASSED")
    return 0
...
def _run_live_smoke(env: dict[str, str]) -> int:
    ...
    if missing:
        print("[evidence-agent-execution] SKIP live smoke (missing " + ", ".join(missing) + ")")
        return 0
    ...
    with tempfile.TemporaryDirectory(prefix="ecgd_live_") as tmpdir:
        ...
        print("[evidence-agent-execution] START live smoke")
        completed = subprocess.run(command, check=False, cwd=ROOT, env=env, capture_output=True, text=True)
        if completed.returncode != 0:
            print(completed.stderr)
            print(f"[evidence-agent-execution] FAIL live smoke (exit={completed.returncode})")
            return completed.returncode
        payload = json.loads(completed.stdout)
        if payload.get("compiler_backend") != "live":
            print("[evidence-agent-execution] FAIL live smoke (compiler_backend != live)")
            return 1
        if not payload.get("compiler_artifact_paths"):
            print("[evidence-agent-execution] FAIL live smoke (missing compiler artifacts)")
            return 1
        print("[evidence-agent-execution] PASS live smoke")
        return 0
