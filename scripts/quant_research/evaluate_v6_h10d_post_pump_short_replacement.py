...
CONTRACT_VERSION = "quant_v6_h10d_post_pump_short_replacement_diagnostic.v1"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate post_pump_stall short replacement / veto rules on the active v6_h10d strategy."
    )
...
def _variant_specs() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "description": (
                "Keep the active v6_h10d score intact, but allow one marginal short replacement "
                "from the bottom-6 pool when a mid-liquidity name has negative post_pump_stall z-score."
            ),
        },
        {
            "label": "replace_mid_v2",
            "candidate_id": "xs_alpha_ontology_v6_spk_short_replace_mid_v2_h10d",
            "base_mechanism_id": "xs_alpha_ontology_v6_spk_short_replace_mid_v2",
            "model_family": "xs_alpha_ontology_v6_h10d_spk_short_replace_mid_v2",
            "manifest_contract_tag": "alpha_ontology_v6_h10d_spk_short_replace_mid_v2",
            "required_feature_columns_append": ["post_pump_stall_core_score_3d"],
            "description": (
                "Stricter short replacement: bottom-8 pool, one replacement max, and only when "
                "post_pump_stall z-score is materially negative (<= -0.5)."
            ),
        },
        {
            "label": "replace_mid_v3",
            "candidate_id": "xs_alpha_ontology_v6_spk_short_replace_mid_v3_h10d",
            "base_mechanism_id": "xs_alpha_ontology_v6_spk_short_replace_mid_v3",
            "model_family": "xs_alpha_ontology_v6_h10d_spk_short_replace_mid_v3",
            "manifest_contract_tag": "alpha_ontology_v6_h10d_spk_short_replace_mid_v3",
            "required_feature_columns_append": ["post_pump_stall_core_score_3d"],
            "description": (
                "More aggressive marginal replacement: bottom-8 pool and up to two short-slot "
                "replacements when mid-liquidity post-pump-stall names are present."
            ),
        },
    ]
...
def _build_candidate_manifest_payload(
    *,
    ...
    lineage["method"] = (
        "SP-K short replacement / veto test on the active v6_h10d strategy. Keep the core-20 universe, "
        "same regime-gating overlay, same top-3/bottom-3 construction; only allow marginal short-slot "
        "replacement near the baseline cutoff when mid-liquidity post-pump-stall names appear in the tail."
    )
    ...
    thesis["market_mechanism"] = (
        "Attach SP-K as a short-selection replacement rule to v6_h10d. Keep the active parent strategy on "
        "`liquid_perp_core_20`; preserve the long leg; only replace marginal baseline short names when a "
        "mid-liquidity post-pump-stall candidate sits just above the short cutoff."
    )
    thesis["directional_claim"] = (
        "The main strategy already identifies the broad short book well. SP-K should help only on the boundary: "
        "do not perturb the whole cross-section, but allow a nearby mid-liquidity post-pump-stall name to replace "
        "a weaker baseline short. Claim fails if walk-forward / regime trade-offs do not improve or at least preserve "
        "the active parent while improving short-basket economics."
    )
    thesis["factor_formula"] = (
        "baseline = v6_h10d_raw; longs unchanged; shorts start from baseline bottom-3. "
        "If a mid-liquidity name in the bottom tail has sufficiently negative post_pump_stall z-score, "
        "it may replace a weaker current short. Final score is re-ranked only to encode that discrete replacement rule."
    )
...
def _selection_change_diagnostic(
    *,
    ...
    return {
        ...
        "entered_mean_post_pump_stall_core_score_3d": _safe_mean(
            entered_df,
            "post_pump_stall_core_score_3d",
        ),
        "exited_mean_post_pump_stall_core_score_3d": _safe_mean(
            exited_df,
            "post_pump_stall_core_score_3d",
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_path = args.output_path or (report_dir / "v6_h10d_post_pump_short_replacement_diagnostic.json")
    ...
    scorer_map = {
        ...
        "replace_mid_v1": xs_alpha_ontology_v6_h10d_spk_short_replace_mid_v1_score,
        "replace_mid_v2": xs_alpha_ontology_v6_h10d_spk_short_replace_mid_v2_score,
        "replace_mid_v3": xs_alpha_ontology_v6_h10d_spk_short_replace_mid_v3_score,
    }
    risk_diagnostics = {
        "baseline_bottom3": _short_risk_diagnostic(
            frame=risk_frame,
            scorer=scorer_map["baseline_v6_h10d"],
            short_count=3,
            target_horizon_bars=args.target_horizon_bars,
        )
    }
    ...
    payload = {
        ...
        "selection_change_diagnostics": selection_diagnostics,
