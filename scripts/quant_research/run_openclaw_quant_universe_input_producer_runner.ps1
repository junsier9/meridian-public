...
. $helperPath
$taskEntry = Get-OpenClawScheduledTaskEntry -RepoRoot $repoRoot -TaskKey "quant_universe_input_producer"
$runnerRoot = Join-Path $env:LOCALAPPDATA "EnhengClaw\quant_research"
$logRoot = Join-Path $runnerRoot "universe_input_producer_runner_logs"
$lockPath = Join-Path $runnerRoot "quant_universe_input_producer.lock"
$cycleWrapper = Join-Path $repoRoot "scripts\quant_research\run_quant_universe_input_producer.py"
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$logPath = Join-Path $logRoot ("openclaw_quant_universe_input_producer_runner_{0}.log" -f $stamp)
...
if (-not (Test-Path $cycleWrapper)) {
    throw "quant universe input producer wrapper not found: $cycleWrapper"
}
...
try {
    ...
    try {
        $pythonExe = Get-OpenClawRepoVenvPythonExecutable -RepoRoot $repoRoot
    } catch {
        Write-Log $_.Exception.Message
        exit 86
    }
    $stdoutCapturePath = Join-Path $runnerRoot "quant_universe_input_producer.stdout.tmp"
    $stderrCapturePath = Join-Path $runnerRoot "quant_universe_input_producer.stderr.tmp"
    Remove-Item -LiteralPath $stdoutCapturePath, $stderrCapturePath -Force -ErrorAction SilentlyContinue

    Write-Log "task=openclaw_quant_universe_input_producer"
