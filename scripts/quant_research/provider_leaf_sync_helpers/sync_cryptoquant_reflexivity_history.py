...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Sync CryptoQuant exchange-flow and market-indicator reflexivity history for M3.2."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    asset_ids = [item.strip().lower() for item in str(args.assets).split(",") if item.strip()]
