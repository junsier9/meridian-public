...
. $helperPath
$taskEntry = Get-OpenClawScheduledTaskEntry -RepoRoot $repoRoot -TaskKey "quant_coinapi_spot_sync"
$runnerRoot = Join-Path $env:LOCALAPPDATA "EnhengClaw\quant_research"
$logRoot = Join-Path $runnerRoot "coinapi_spot_sync_runner_logs"
$lockPath = Join-Path $runnerRoot "quant_coinapi_spot_sync.lock"
$cycleWrapper = Join-Path $repoRoot "scripts\quant_research\run_quant_coinapi_spot_sync.py"
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$logPath = Join-Path $logRoot ("openclaw_quant_coinapi_spot_sync_runner_{0}.log" -f $stamp)
...
if (-not (Test-Path $cycleWrapper)) {
    throw "quant CoinAPI spot sync wrapper not found: $cycleWrapper"
}
...
try {
    ...
    $stdoutCapturePath = Join-Path $runnerRoot "quant_coinapi_spot_sync.stdout.tmp"
    $stderrCapturePath = Join-Path $runnerRoot "quant_coinapi_spot_sync.stderr.tmp"
    Remove-Item -LiteralPath $stdoutCapturePath, $stderrCapturePath -Force -ErrorAction SilentlyContinue

    Write-Log "task=openclaw_quant_coinapi_spot_sync"
