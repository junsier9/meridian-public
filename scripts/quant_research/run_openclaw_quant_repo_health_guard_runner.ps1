...
. $helperPath
$taskEntry = Get-OpenClawScheduledTaskEntry -RepoRoot $repoRoot -TaskKey "quant_repo_health_guard"
$runnerRoot = Join-Path $env:LOCALAPPDATA "EnhengClaw\\quant_research"
$logRoot = Join-Path $runnerRoot "repo_health_guard_logs"
$lockPath = Join-Path $runnerRoot "quant_repo_health_guard.lock"
$guardWrapper = Join-Path $repoRoot "scripts\\verify\\run_quant_repo_health_guard.py"
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$logPath = Join-Path $logRoot ("openclaw_quant_repo_health_guard_{0}.log" -f $stamp)
...
if (-not (Test-Path $guardWrapper)) {
    throw "quant repo health guard wrapper not found: $guardWrapper"
}
...
try {
    ...
    $stdoutCapturePath = Join-Path $runnerRoot "quant_repo_health_guard.stdout.tmp"
    $stderrCapturePath = Join-Path $runnerRoot "quant_repo_health_guard.stderr.tmp"
    Remove-Item -LiteralPath $stdoutCapturePath, $stderrCapturePath -Force -ErrorAction SilentlyContinue

    Write-Log "task=openclaw_quant_repo_health_guard"
    Write-Log "repo_root=$repoRoot"
    Write-Log "as_of=$effectiveAsOf"
    Write-Log "wrapper=$guardWrapper"
