...
. $helperPath
$taskEntry = Get-OpenClawScheduledTaskEntry -RepoRoot $repoRoot -TaskKey "quant_universe_freeze"
$runnerRoot = Join-Path $env:LOCALAPPDATA "EnhengClaw\\quant_research"
$logRoot = Join-Path $runnerRoot "universe_freeze_runner_logs"
$lockPath = Join-Path $runnerRoot "quant_universe_freeze.lock"
$cycleWrapper = Join-Path $repoRoot "scripts\\quant_research\\run_quant_universe_freeze.py"
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$logPath = Join-Path $logRoot ("openclaw_quant_universe_freeze_runner_{0}.log" -f $stamp)
...
if (-not (Test-Path $cycleWrapper)) {
    throw "universe freeze wrapper not found: $cycleWrapper"
}
...
try {
    ...
    $stdoutCapturePath = Join-Path $runnerRoot "quant_universe_freeze.stdout.tmp"
    $stderrCapturePath = Join-Path $runnerRoot "quant_universe_freeze.stderr.tmp"
    Remove-Item -LiteralPath $stdoutCapturePath, $stderrCapturePath -Force -ErrorAction SilentlyContinue

    Write-Log "task=openclaw_quant_universe_freeze"
