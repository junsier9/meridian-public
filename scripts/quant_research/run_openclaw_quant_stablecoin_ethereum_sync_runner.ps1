...
. $helperPath
$taskEntry = Get-OpenClawScheduledTaskEntry -RepoRoot $repoRoot -TaskKey "quant_stablecoin_ethereum_sync"
$runnerRoot = Join-Path $env:LOCALAPPDATA "EnhengClaw\quant_research"
$logRoot = Join-Path $runnerRoot "stablecoin_ethereum_sync_runner_logs"
$lockPath = Join-Path $runnerRoot "quant_stablecoin_ethereum_sync.lock"
$cycleWrapper = Join-Path $repoRoot "scripts\quant_research\run_quant_stablecoin_ethereum_sync_cycle.py"
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$logPath = Join-Path $logRoot ("openclaw_quant_stablecoin_ethereum_sync_runner_{0}.log" -f $stamp)
...
if (-not (Test-Path $cycleWrapper)) {
    throw "stablecoin sync wrapper not found: $cycleWrapper"
}
...
try {
    ...
    $stdoutCapturePath = Join-Path $runnerRoot "quant_stablecoin_ethereum_sync.stdout.tmp"
    $stderrCapturePath = Join-Path $runnerRoot "quant_stablecoin_ethereum_sync.stderr.tmp"
    Remove-Item -LiteralPath $stdoutCapturePath, $stderrCapturePath -Force -ErrorAction SilentlyContinue

    Write-Log "task=openclaw_quant_stablecoin_ethereum_sync"
